...
class CursorConnectorRuntime:
    ...
    def adapter_verification_notice(self) -> ConnectorRuntimeNotice | None:
        return ConnectorRuntimeNotice(
            ...
            reason_code="plan_allowance_possible",
            message=(
                "Adapter verification may consume a small amount of this connector's "
                "plan allowance."
            ),
            recoverable=True,
    ...
    async def discover(self, context: ConnectorExecutionContext) -> DiscoveryResult:
        ...
        if not executable:
            return DiscoveryResult(
                ...
                details={"detail": "cursor-agent not found", "node_id": context.node_id},
            )
        ...
        if version is None:
            return DiscoveryResult(
                ...
                details={
                    "node_id": context.node_id,
                    "detail": "unable to parse Cursor version",
                },
            )
        return DiscoveryResult(
            executable_path=executable,
    ...
    async def verify_authentication(
        ...
        return AuthenticationEvidence(
            ...
            method_id=method.id,
            executable_path=executable,
            fingerprint=executable_fingerprint(shutil.which(executable) or executable),
    ...
    def classify_auth_status(self, output: str, *, returncode: int) -> str:
        return (
            "authenticated"
            if parse_cursor_auth_status(output, returncode=returncode)
    ...
    async def verify_adapter(self, context: ConnectorExecutionContext) -> AdapterVerificationResult:
        ...
        notice = self.adapter_verification_notice()
        return AdapterVerificationResult(
            passed=passed,
            ...
            details={
                ...
                "node_id": context.node_id,
                "notice": notice.as_payload() if notice is not None else None,
            },
        )
    ...
    async def execute(
        ...
        for item in self.parse_events(output):
            ...
            yield HarnessEvent(
                ...
                sequence=sequence,
                payload={"cursor_event": item.get("type"), "redacted": True},
            )
        sequence += 1
        yield HarnessEvent(
    ...
    async def _probe_version_and_health(self, executable: str) -> tuple[str | None, str | None]:
        ...
        if process.returncode != 0 and (
            ...
            return None, "Cursor launcher is broken or not executable"
        if not combined:
            return None, None
