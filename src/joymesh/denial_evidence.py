...
def parse_denials(text: str, *, pid: int, started_ns: int, ended_ns: int) -> list[dict[str, Any]]:
    events = []
    for line in text.splitlines():
        try:
            item = json.loads(line)
            if (
                item.get("processID") != 0
                or item.get("processImagePath") != "/kernel"
            ...
            events.append(
                {
                    ...
                    "operation": match[2],
                    "observed_ns": ns,
                    "source": "macos-kernel-sandbox",
                    "boot_uuid": item.get("bootUUID"),
                }
            )
...
def collect_denials(receipt: dict[str, Any]) -> dict[str, Any]:
    """Post-stop diagnostic: missing records are explicitly NOT proof of no denial."""
    pid = int(receipt["pid"])
    predicate = (
        'processID == 0 AND senderImagePath ENDSWITH "/Sandbox" '
        f'AND eventMessage CONTAINS "({pid}) deny("'
    )
